...
const processLessToCSS = () => {
    ...
    var src = gulp.src( [
            `./src/styles/${config.projectName}/*.less`
        ] )
...
const deployCSS = ( callback ) => {
    let src = gulp.src( `./src/styles/${config.projectName}-*.css` )
...
const watchLess = () => {
    ...
    if ( config.WebDAV ) {
         ...
         */
        if ( ! config.uploadSource ) lessFileGlob.push( `!./src/styles/${config.projectName}/**/*.less` )
     ...
     * @fires makeCSS
     */
    gulp.watch( `./src/styles/${config.projectName}/**/*.less`, makeCSS )
}
...
const watchImages = () => {
    ...
    if ( config.WebDAV ) {
        gulp.watch( [
            './src/preview.{jpg,png}',     // Preview theme images.
            './src/images/**/*.{jpg,png}', // All template images.
...
const watchSmarty = () => {
    ...
    if ( config.WebDAV ) {
        gulp.watch( [
            './src/scripts/**/*.tpl', // Template files.
            './src/boxes/**/*.tpl',   // Box template files.
